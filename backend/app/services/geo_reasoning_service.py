...
"""
Usage:
    service = GeoReasoningService(three_level_service)
...
class GeoReasoningService:
    ...
    def __init__(self, three_level_service: ThreeLevelService):
    ...
    def parse_sql_response(self, raw_text: str) -> Dict[str, Any]:
        ...
        response = self.parse_json_response(raw_text)
        if (
            "final_sql" not in response
            or "assumptions" not in response
            or "notes" not in response
        ):
    ...
    def start_geo_reasoning(
        self, user_question: str, constraints: Optional[Dict[str, Any]] = None
        ...
        l1_response = llm_service.generate(l1_prompt)
        l1_selected = self.parse_l1_response(l1_response.content).get("l1_selected")
        ...
        l2_response = llm_service.generate(l2_prompt)
        l2_selected = self.parse_l2_response(l2_response.content).get("l2_selected")
        ...
        l3_response = llm_service.generate(l3_prompt)
        l3_selected = self.parse_l3_response(l3_response.content).get("l3_selected")

        l3_schema = []
        for l3 in l3_selected:
            l3_schema.extend(self.three_level.get_l3_table_schema(l3["id"]))
        sql_prompt = self.build_sql_prompt(
            user_question, l3_selected, l3_schema, constraints
        )
        sql_response = llm_service.generate(sql_prompt)
        sql_result = self.parse_sql_response(sql_response.content).get("final_sql")
