...
def test_routing_step_by_step():
    ...
    with app.app_context():
        ...
        try:
            ...
            r1 = app.llm_service.generate(prompt=s1_user, system_prompt=s1_sys)
            ...
            r2 = app.llm_service.generate(prompt=s2_user, system_prompt=s2_sys)
            ...
            r3 = app.llm_service.generate(prompt=s3_user, system_prompt=s3_sys)
            ...
            if table_name:
                ...
                s4_sys, s4_user = svc._build_step4_prompt(user_question, l3_selected, l3_schema, constraints)
                r4 = app.llm_service.generate(prompt=s4_user, system_prompt=s4_sys)
