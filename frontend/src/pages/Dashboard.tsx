...
export default function Dashboard() {
  ...
  // Navigate to Results with current query; also stash a fallback
  function goToResult() {
    const q = (result?.queryEcho || query || "").trim();
    if (!q) return;
    sessionStorage.setItem("lastQuery", q);
    const url = `/result?q=${encodeURIComponent(q)}`;
          ...
          <div className="flex-1 relative">
            <div className="absolute inset-0 rounded-lg m-4 shadow-lg overflow-hidden">
              ...
              <div className="w-full h-full flex items-center justify-center relative bg-gradient-to-br from-[hsl(var(--gradient-start))/0.15] to-[hsl(var(--gradient-end))/0.15] dark:from-[hsl(var(--gradient-end))/0.25] dark:to-[hsl(var(--primary))/0.25]">
                ...
                {/* View detailed output button (bottom-right) */}
                <div className="absolute bottom-4 right-4 flex items-center gap-2">
                  <Button
                    onClick={goToResult}
                    disabled={running || (!result && !query.trim())}
                    className="shadow-lg"
                  >
